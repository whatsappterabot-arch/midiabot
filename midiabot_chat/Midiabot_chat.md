...
- **Não existe hoje relação de posse sala↔vendedor separada** — a única menção (`midiabot_midiachat_sala_vendedor`, N-pra-N) está só na documentação do MidiaChat, pra outra finalidade (destacar aba do dono no `chat.html`), e nenhuma tela do painel administrativo lê/grava isso. A posse de uma sala dedicada será **derivada**: sender atrelado à sala (`midiabot_sender_chatid`, via "Conectores de Chat") + esse sender atribuído a um vendedor (`vendedores.html`, "Telefones dos Consultores") = esse vendedor é o dono, por transitividade. Isso não afeta `midiabot_remotejid_chatid` (atribuição de conversa a sala), que é um mecanismo totalmente separado.
- **Regra de "apagar sala"** (pendência antiga, revisada): fica com **2** condições, não 3 — sem sender atrelado e sem conversa atribuída (`midiabot_remotejid_chatid`). "Sem vendedor dono" não é checagem separada, já está coberta por "sem sender" (dado que posse é derivada do sender).
...
2. **"Atribuição a Consultores" (`consultores.html`)**: rescopear de `workflow_name` pra `chat_id` — precisa antes descobrir/confirmar a tabela real por trás de `buscar_config`/`salvar` dessa tela (ainda não levantada).
3. **"Telefones dos Consultores" (`vendedores.html`)**: trocar o campo `sender` de texto livre pra um `<select>`, mostrando **"NomeDaInstância / número"** (não o `sender` cru), listando só senders de salas **não** compartilhadas (`sala_compartilhada = 0`) — o sender já salvo daquele vendedor precisa continuar aparecendo mesmo se a regra mudasse depois. Texto de ajuda da tela precisa explicar em linguagem simples por que alguns números não aparecem (números de sala compartilhada são divididos automaticamente, não fazem sentido atribuir a um consultor só).

## Acesso somente-leitura ao banco para o Claude (2026-08-08, em configuração)

Decisão: dar ao Claude acesso **só leitura** ao Postgres (nunca escrita), via servidor MCP, pra reduzir idas e vindas de "roda essa query e cola o resultado" — mudanças no banco continuam exigindo o usuário rodar manualmente, sempre. Host: `awkwardgiantpanda-postgres.cloudfy.live`, banco `postgres` (confirmado via `SELECT current_database()`), porta padrão 5432. Passos passados ao usuário: criar usuário `claude_readonly` (só `SELECT`, com `ALTER DEFAULT PRIVILEGES` pra cobrir tabelas futuras automaticamente) e registrar via `claude mcp add midiabot-db -- npx -y @modelcontextprotocol/server-postgres "postgresql://claude_readonly:SENHA@awkwardgiantpanda-postgres.cloudfy.live:5432/postgres"` num terminal separado (não colado no chat, por segurança). **Conclusão não confirmada ainda** — precisa reiniciar o Claude Code depois de rodar o comando.
