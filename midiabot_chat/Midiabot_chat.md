...
**Plano combinado em 2026-08-07, antes de uma pausa**: o fluxo de n8n cresceu com bastante remendo incremental nesta sessão (Switch novo, várias branches no Code node, tratamento de edição que não guarda nada) — antes de seguir construindo em cima, a ideia é reconstruir/organizar esse fluxo, avaliar o que precisa mudar no painel administrativo por causa disso, e só depois voltar a atacar o envio de mídia/tipos de mensagem do lado do vendedor pro WhatsApp (a "devolução das mensagens do chat pro WhatsApp").

- **Reconstruir/organizar o fluxo de recebimento** no n8n (workflow "Publi ScentyStore v1"), consolidando os remendos desta sessão (Switch de filtro de evento, Code node com muitas branches) — próximo passo combinado, ainda não iniciado.
- Avaliar o que precisa mudar no **painel administrativo** (fora de `/midiabot_chat/`) por causa das mudanças de esquema/lógica desta sessão — ainda não avaliado especificamente.
- **Envio de mídia e outros tipos de mensagem do lado do vendedor** (upload de arquivo, resposta citando mensagem) — reconhecido como pendência há tempo, ainda não iniciado; combinado que só começa depois da reconstrução do fluxo acima.
...
- **"Conectores de Chat" (`telegram_sender.html`) não permite mover uma sala de um workflow pra outro nem apagar uma sala** — só existe criar sala (workflow fixo na criação) e atrelar/desatrelar sender. Decisão (2026-08-07): construir "apagar sala", condicionado a **três** checagens, não só uma — sem sender atrelado, sem nenhuma conversa atribuída (`midiabot_remotejid_chatid`) e sem vendedor dono (`midiabot_midiachat_sala_vendedor`); histórico antigo em `midiabot_historico_mensagens` não bloqueia o apagar. Não iniciado ainda.
