...
6. **Checar se `enviar_mensagem` (envio pra cliente externo) tem o mesmo bug de vírgula** encontrado e corrigido no `enviar_mensagem_interna` (2026-08-17/18): o campo "Query Parameters" do n8n, no formato de texto único separado por vírgula, corrompe qualquer parâmetro de texto livre que tenha vírgula dentro — mensagem de cliente real tem vírgula com frequência, então é candidato forte a estar com esse bug hoje em produção. Ver `[[feedback_n8n_comma_parameter_bug]]` na memória pro conserto (trocar pelo formato `{{ [ ... ] }}`).
