...
- **`git push` deste repositório precisa da conta GitHub `whatsappterabot-arch`, não a pessoal (`mtannuri`)**: o remoto `origin` já está com essa conta embutida na URL (`https://whatsappterabot-arch@github.com/whatsappterabot-arch/midiabot.git`). A autenticação é feita via **GitHub CLI (`gh`)**, configurado como credential helper do Git pra `github.com` (`credential.https://github.com.helper` no `.gitconfig` global, `gh auth git-credential`) — nunca senha digitada, o `gh` já tem as duas contas (`mtannuri` e `whatsappterabot-arch`) autenticadas e guardadas no keyring do Windows. Se o `git push` falhar com erro de permissão/403 ou pedindo senha num terminal, o motivo mais provável é a conta **ativa** do `gh` estar errada — resolve com `gh auth switch --hostname github.com --user whatsappterabot-arch` (isso é uma troca **global** na máquina, afeta qualquer outro projeto que espere a conta `mtannuri` ativa por padrão; reverter com `gh auth switch --hostname github.com --user mtannuri`).
